...
func parseString(in string) (string, int) {
	re_s := regexp.MustCompile(`[UDLR]`)
	re_n := regexp.MustCompile(`[0-9]+`)

	s := re_s.FindString(in)
	n, _ := strconv.Atoi(re_n.FindString(in))

	return s, n
}

func getInput(input string) (res [][]string) {
...
func getLine(arr []string) (line []Point) {
	p := Point{0, 0, 0}

	for _, str := range arr {
		direction, step := parseString(str)
...
func getDistanceAndStep(input string) (int, int) {
  ...
  var all_steps []int
	...
	for _, arr := range getInput(input) {
    line_steps := make(map[[2]int]int)
    line_count := make(map[[2]int]int)
		for _, point := range getLine(arr) {
      xy := [2]int{point.x, point.y}
      if line_count[xy] != 1 {
        line_count[xy] = 1
      }
      if line_steps[xy] == 0 || point.z < line_steps[xy] {
        line_steps[xy] = point.z
      }
		}
    for k, _ := range line_count {
      count[k] += 1
    }
    for k, v := range line_steps {
      steps[k] += v
    }
	}
    ...
    all_steps = append(all_steps, steps[p])
		distances = append(distances, absInt(p[0]) + absInt(p[1]))
	}

	return minInts(distances), minInts(all_steps)
}

func test() {
  d1, s1 := getDistanceAndStep(input1)
  d2, s2 := getDistanceAndStep(input2)
  if d1 != 159 {
    fmt.Println("Failed input 1 part 1")
  }
  if s1 != 610 {
    fmt.Println("Failed input 1 part 2")
  }
  if d2 != 135 {
    fmt.Println("Failed input 2 part 1")
  }
  if s2 != 410 {
    fmt.Println("Failed input 2 part 2")
  }
  return
}
