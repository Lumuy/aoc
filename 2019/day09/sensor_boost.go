...
func fetchValue(data map[int]int, rbase, val, mode int) (res int) {
  switch mode {
  case 0:
    res = data[val]
  case 1:
...
func process(data map[int]int, in int) string {
	var res []string
	var hang bool
	var halts bool
  var rbase int
  var pointer int
	...
	for !halts {
		...
		switch de {
		case 1:
			l := fetchValue(data, rbase, data[pointer+1], c)
			r := fetchValue(data, rbase, data[pointer+2], b)
			d := data[pointer+3]
		...
		case 4:
      out := fetchValue(data, rbase, data[pointer+1], c)
			res = append(res, strconv.Itoa(out))
			pointer += 2
		case 5:
			v1 := fetchValue(data, rbase, data[pointer+1], c)
			v2 := fetchValue(data, rbase, data[pointer+2], b)
			if v1 != 0 {
				pointer = v2
			} else {
				pointer += 3
